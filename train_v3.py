...
손실함수 구성:
    ...
    attention_loss = MSE(P3_heatmap, rtdetr_small_heatmap)   ← 소형 객체
                   + MSE(P4_heatmap, rtdetr_mid_heatmap)     ← 중형 객체
                   + MSE(P5_heatmap, rtdetr_large_heatmap)   ← 대형 객체
...
def make_yolo_heatmap(feat: torch.Tensor) -> torch.Tensor:
    """
    YOLO feature map → heatmap (최대값 정규화)
    feat   : (B, C, H, W)
    return : (B, H*W)  0~1
    """
    B = feat.shape[0]
...
def make_rtdetr_heatmap(heatmap: torch.Tensor) -> torch.Tensor:
    """
    CrossAttnWeightHook 출력 → 최대값 정규화
    heatmap : (B, G*G)
    return  : (B, G*G)  0~1
    """
    return heatmap / (heatmap.max(dim=1, keepdim=True).values + 1e-8)
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # ★ 변경 5 : 크기별 Attention Loss 합산
            # 소형 쿼리 → P3, 중형 쿼리 → P4, 대형 쿼리 → P5
            attn_loss = (
                F.mse_loss(yolo_small, rtdetr_small.detach()) +
                F.mse_loss(yolo_mid,   rtdetr_mid.detach())   +
                F.mse_loss(yolo_large, rtdetr_large.detach())
            )
